...
HINT: One of these apeproaches uses ROW_NUMBER() and one uses DENSE_RANK(). */
...
SELECT *
	,row_number () OVER (ORDER BY daily_sales DESC) as best_day
	,row_number () OVER (ORDER BY daily_sales) as worst_day
FROM sales_by_date;
